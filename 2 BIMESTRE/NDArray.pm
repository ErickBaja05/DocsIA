...
method slice(Slice|AdvancedSlice|InternalSlice|Undef @slices){
    ...
    #}
    if(grep { defined($_) and not ref($_) and /^(?:begin|end|slice)$/ } @slices){
        my %args = @slices;
        
        if(exists $args{begin} || exists $args{end}){
...
method asscalar()
{
    ...
    if (ref($self) eq 'AI::MXNet::NDArray') {
...
method asnumperl()
{
    ...
    $pdl->upd_data;
    return new_numperl('numperl', $pdl->unpdl);
}

sub new_numperl {
  my ($that, $self) = (shift, shift);
...
sub  _ufunc_helper
{
    ...
    # Converts AI::MXNet::NDArray::Slice into AI::MXNet::NDArray
    $lhs = $lhs->sever
        if blessed($lhs)
        and $lhs->isa('AI::MXNet::NDArray::Slice');
    
    $rhs = $rhs->sever
        if blessed($rhs)
        and $rhs->isa('AI::MXNet::NDArray::Slice');
